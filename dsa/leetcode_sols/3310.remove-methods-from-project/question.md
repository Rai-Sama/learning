# [3310. Remove Methods From Project][link] (Medium)
...
= [aᵢ, bᵢ]` indicates that method `aᵢ` invokes method `bᵢ`.
...
You may return the answer in any order. If it is not possible to remove **all** the suspicious
...
Method 2 and method 1 are suspicious, but they are directly invoked by methods 3 and 0, which are
not suspicious. We return all elements without removing anything.
...
Methods 0, 1, and 2 are suspicious and they are not directly invoked by any other method. We can
remove them.
